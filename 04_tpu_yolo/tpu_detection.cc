...
#define IMAGE_PATH "/home/byeongyun/EAI_TfLite/04_tpu_yolo/image/sample2.jpg" 
...
int main(int argc, char* argv[]) {
  ...
  for(int seq=0; seq<inference_num; ++seq){
    ...
    const uint8_t* src_data = input.ptr<uint8_t>();


    for (size_t i = 0; i < 448 * 448 * 3; ++i) {
      int c = i % 3;
      float pixel = static_cast<float>(src_data[i]) / 255.0f;
      float normalized = (pixel - MEAN[c]) / STD[c];
      input_tensor[i] = static_cast<int8_t>(std::round(normalized / input_scale) + input_zero_point);
    }
    ...
    for (int y = 0; y < S; ++y) {
        for (int x = 0; x < S; ++x) {
            ...
            std::vector<float> class_logits(C);
            for (int c = 0; c < C; ++c) {
                int8_t raw = output_tensor[cell_offset + c];
                float dequant = (static_cast<float>(raw) - output_zero_point) * output_scale;
                class_logits[c] = dequant; 
            }
            float max_logit = -1e9f;
            for (int c = 0; c < C; ++c) {
                if (class_logits[c] > max_logit) max_logit = class_logits[c];
            }
            float sum_exp = 0.f;
            for (int c = 0; c < C; ++c) {
                sum_exp += expf(class_logits[c] - max_logit);
            }
            std::vector<float> class_probs(C);
            for (int c = 0; c < C; ++c) {
                class_probs[c] = expf(class_logits[c] - max_logit) / std::max(sum_exp, 1e-12f);
            }
            ...
            for (int b = 0; b < B; ++b) {
                ...
                int max_class = -1;
                for (int c = 0; c < C; ++c) {
                    if (class_probs[c] > max_prob) {
                        max_prob = class_probs[c];
                        max_class = c;
                    }
                }

                float prob = dequant_conf * max_prob;
                ...
                if (prob > SCORE_THRESH) {
                        ...
                        float w = sigmoid(dequant_w);
                        float h = sigmoid(dequant_h);
                        w = w * w;
                        h = h * h;
                        ...
                        boxes.push_back({cx, cy, w, h, dequant_conf, max_class, prob});
                }
            }
        }
    }
